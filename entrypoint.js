                ...
                if (secureFunction /** enabled */ && functionInfo.KMSKeyArn) {
                    ...
                    simplify.updateFunctionConfiguration({
                        ...
                        simplify.enableOrDisableLogEncryption({
                            ...
                            simplify.consoleWithMessage(opName, `${cmdOPS} ${functionInfo.FunctionName} : ${(logRetention ? `Configured LogGroup retention for ${logRetention} days!` : `LogGroup is not required to configure`)} \x1b[32m (OK) \x1b[0m`)
                            resolve(args)
                        }).catch(function (err) {
                            reject(`${err}`)
                        })
